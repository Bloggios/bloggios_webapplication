...
import {addQuestion} from "../../restservices/QuestionApi";
...
const QuestionSubmitModal = ({
    ...
    useLayoutEffect(() => {
        let tips = [];
        if (isModelOpen) {
            const split = data.title.split(/\s+/);
            if (split.length > 16) {
                tips.push('Try to express your question title in fewer words for maximum impact?');
            } else if (split.length <= 2) {
                tips.push('Try to express your question title in more words for maximum impact?');
            }
            if (data.tags && data.tags.length === 0) {
                tips.push('Adding tags to categorize the question and help with reachability.');
            } else if (data.tags && data.tags.length === 1) {
                tips.push('If applicable, add more tags to help with reachability');
            }
            if (data.detailsText && data.detailsText.length !== 0) {
                const words = data.detailsText.split(/\s+|\\n/);
                if (words.length < 5) {
                    tips.push('The more details you provide, the better user can understand your question');
                }
            } else if (!data.detailsText) {
                tips.push('The more details you provide, the better user can understand your question');
            }
            if (data.images && data.images.length === 0) {
                tips.push('You can include an example (photo or screenshot) to illustrate your question?')
            } else if (!data.images) {
                tips.push('You can include an example (photo or screenshot) to illustrate your question?')
            }
        }
        if (tips && tips.length > 0) {
            setBloggiosTips(tips);
        }
    }, [data]);

    const addQuestionMutation = useMutation({
        mutationFn: (formData) => addQuestion(formData),
    ...
    const handleSubmit = () => {
        ...
        isFormDataAdded && addQuestionMutation.mutate(formData);
    }
        ...
        >
            <Wrapper>
                ...
                <Tips>
                    ...
                    {bloggiosTips.length > 0 && (
                        <div>
                            <h6>Bloggios Tips:</h6>
                            <ul>
                                {bloggiosTips.map((data, index) => (
                ...
                <ButtonsWrapper>
                    <IconButton
                        ...
                        tooltipId={'question-model-submit-icon-button'}
                    ...
                    <IconButton
                        ...
                        hColor={colors.white100}
                        aColor={colors.white100}
                        tooltipId={'question-model-cancel-icon-button'}
